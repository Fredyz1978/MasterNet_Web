...
SET ANSI_NULLS ON
GO
CREATE PROC [PRODUCCION].[PR_SELECT_PRODUCTO_CODIGO]
...
select PR_PRO_COD, PR_PRO_NOM, ISNULL(CO_KAR_TOT_ENT_VAL_UNI,0) as PR_PRO_PRE, ISNULL(CO_KAR_TOT_EXI_CANT,0) as PR_PRO_CAN, PR_TIP_REC_COD, PR_PRO_UNI, PR_PRO_PES
from (
		SELECT RANK() OVER (PARTITION BY PR_PRO_COD ORDER BY CO_KAR_TOT_COD desc) AS 'RowNumber2', PR_PRO_COD, CO_KAR_TOT_EXI_CANT, 
				CO_KAR_TOT_ENT_VAL_UNI, PR_PRO_NOM, PR_TIP_REC_COD, PR_PRO_UNI, PR_PRO_PES
		FROM S
		WHERE RowNumber=1
) as X
where RowNumber2=1
