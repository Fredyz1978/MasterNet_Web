...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[CO_DOCUMENTO_COMPRA_REOC]
@mes int,
@year int,
@CO_EMP_RUC CHAR(13)
AS
select p.CO_TIP_IDE_PRO_COD as tpIdProv, p.CO_PRO_CED as idProv, dc.CO_TIP_COM_COM_COD as tipoComp, 
		...
		dc.co_doc_com_num, dc.CO_DOC_COM_IVA, dc.CO_DOC_COM_TOT_PAG, CO_DOC_COM_ID 
from co_documento_compra dc inner join co_proveedor p on dc.CO_PRO_COD = p.CO_PRO_COD
WHERE month(dc.CO_DOC_COM_FEC_EMI)=@mes and year(dc.CO_DOC_COM_FEC_EMI)=@year AND DC.CO_EMP_RUC=@CO_EMP_RUC
GO
